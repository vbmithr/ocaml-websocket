...
let client ?(name = "websocket.client") ?(extra_headers = Header.init ())
  ...
  let drain_handshake r w =
    ...
    >>= fun () ->
    Request.write (fun _ -> Deferred.unit) req w
    >>= fun () ->
    Response.read r
    >>= function
    ...
    | `Ok response ->
        ...
        if Code.(is_error @@ code_of_status status) then
          Reader.contents r
          >>= fun msg ->
          Logs_async.err ~src (fun m -> m "%s" msg)
          >>= fun () ->
        ...
        else Deferred.unit in
  let run () =
    drain_handshake net_to_ws ws_to_net
    >>= fun () ->
    ...
    let rec forward_frames_to_app ws_to_app =
      ...
      >>= fun () -> forward_frames_to_app ws_to_app in
    let forward_frames_to_net ws_to_net app_to_ws =
      Writer.transfer ws_to_net app_to_ws (fun fr ->
          ...
          Writer.write ws_to_net contents ) in
    Deferred.any_unit
      [ forward_frames_to_app ws_to_app;
        forward_frames_to_net ws_to_net app_to_ws;
        Deferred.all_unit Pipe.[closed app_to_ws; closed ws_to_app] ] in
  let finally_f = lazy (Pipe.close_read app_to_ws ; Pipe.close ws_to_app) in
  Monitor.try_with_or_error ~name run >>| fun res -> Lazy.force finally_f ; res
...
let client_ez ?opcode ?(name = "websocket.client_ez") ?extra_headers ?heartbeat
  ...
  let send_ping w span =
    ...
    then Lazy.force cleanup in
  let react w fr =
    let open Frame in
    Logs_async.debug ~src (fun m -> m "<- %a" Frame.pp fr)
    >>= fun () ->
    ...
    | Opcode.Close ->
        ...
        >>| fun () -> Pipe.close w ; None
    | Opcode.Pong ->
        last_pong := Time_ns.now () ;
        return None
    | Opcode.Text | Opcode.Binary -> return @@ Some fr.content
    | _ -> Pipe.write w @@ Frame.close 1002 >>| fun () -> Pipe.close w ; None
  in
...
let server ?(name = "websocket.server")
  ...
  let handshake r w =
    ...
    >>= (function
          ...
          | false ->
              ...
              write ~flush:true (fun w -> write_body w body) response w
              >>= fun () -> raise Exit )
    >>= fun () ->
      ...
      Option.value_map (Header.get headers "sec-websocket-protocol") ~default:[]
        ~f:(fun p ->
          Option.value_map (select_protocol p) ~default:[] ~f:(fun selected ->
              [("Sec-WebSocket-Protocol", selected)] ) ) in
    ...
    let response =
      Cohttp.Response.make ~status:`Switching_protocols
        ...
        () in
    Response.write (fun _ -> Deferred.unit) response w in
  ...
  |> Deferred.Or_error.bind ~f:(fun () ->
         ...
         Monitor.protect
           ~finally:(fun () ->
             Pipe.close ws_to_app ; Pipe.close_read app_to_ws ; Deferred.unit )
           (fun () ->
             Deferred.any
               [ transfer_end; loop (); Pipe.closed ws_to_app;
...
let upgrade_connection ?(select_protocol = fun _ -> None)
  ...
  let handler reader writer =
    ...
    let rec loop () =
      ...
      | Ok frame -> Pipe.write ws_to_app frame >>= loop in
    let buf = Buffer.create 128 in
    ...
    let ping () =
      if Time_ns.Span.(ping_interval = zero) then Deferred.never ()
        ...
        ping_loop () in
    let transfer_end () =
      Pipe.transfer app_to_ws Writer.(pipe writer) ~f:frame_to_string in
    let finally () =
      Pipe.close ws_to_app ; Pipe.close_read app_to_ws ; Deferred.unit in
    Monitor.protect ~finally (fun () ->
        set_tcp_nodelay writer ;
        Deferred.all_unit
          [ Deferred.any
              [ transfer_end (); loop (); ping (); Pipe.closed ws_to_app;
                Pipe.closed app_to_ws ]; f () ] ) in
  (response, handler)
