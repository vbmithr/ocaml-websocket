...
let () = Logs.set_reporter (Logs_async_reporter.reporter ())
  ...
  let tcp_fun (r, w) =
    ...
    let extra_headers =
      Option.value_map protocol ~default:extra_headers ~f:(fun proto ->
          C.Header.add extra_headers "Sec-Websocket-Protocol" proto ) in
    let extra_headers =
      Option.value_map extensions ~default:extra_headers ~f:(fun exts ->
          C.Header.add extra_headers "Sec-Websocket-Extensions" exts ) in
    let r, w =
...
let handle_client addr reader writer =
  ...
  Logs_async.info ~src (fun m -> m "Client connection from %s" addr_str)
  >>= fun () ->
  ...
  let rec loop () =
    ...
    >>= function
    | `Eof ->
        Logs_async.info ~src (fun m -> m "Client %s disconnected" addr_str)
    | `Ok ({Frame.opcode; content; _} as frame) ->
        let open Frame in
        Logs_async.debug ~src (fun m -> m "<- %a" Frame.pp frame)
        >>= fun () ->
        let frame', closed =
          match opcode with
          ...
          | _ -> (Some (close 1002), false) in
        ( match frame' with
        | None -> Deferred.unit
        | Some frame' ->
            Logs_async.debug ~src (fun m -> m "-> %a" pp frame')
            >>= fun () -> Pipe.write sender_write frame' )
        >>= fun () -> if closed then Deferred.unit else loop () in
  Deferred.any
...
let command =
  ...
  let run protocol extension loglevel is_server url () =
    ...
    | true ->
        let port =
          Option.value_exn ~message:"no port inferred from scheme"
